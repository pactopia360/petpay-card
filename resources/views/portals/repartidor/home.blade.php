@php
    ...
    $profileDataLocked = in_array(
        $identityProfile->status ?? 'draft',
        ['submitted', 'under_review', 'approved', 'corrections_required', 'rejected'],
        true
    );
...
<div class="driver-section-heading">
            <h2>Datos complementarios del asociado</h2>
            <p>Completa la información pendiente de tu expediente.</p>
        </div>
        ...
        >
            ...
            {{-- Información registrada: visible, protegida y no editable directamente. --}}
            <div class="driver-form-grid driver-form-grid--3">
                <label>
                    <span>Nombre <b>*</b> <small>🔒 Protegido</small></span>
                ...
                <label>
                    <span>Apellido paterno <b>*</b> <small>🔒 Protegido</small></span>

                    <input
                        type="text"
                        name="paternal_last_name"
                        value="{{ $identityProfile->paternal_last_name }}"
                        readonly
                        required
                    >
                </label>
                ...
                <label>
                    <span>Apellido materno <small>🔒 Protegido</small></span>

                    <input
                        type="text"
                        name="maternal_last_name"
                        value="{{ $identityProfile->maternal_last_name }}"
                        readonly
                    >
                </label>
            </div>

            <div class="driver-protected-data-notice">
                <div>
                    <span aria-hidden="true">🔒</span>

                    <div>
                        <strong>Datos de registro protegidos</strong>
                        <small>
                            Nombre, apellidos y datos de acceso provienen de tu registro inicial.
                            Para corregirlos deberás solicitar una actualización.
                        </small>
                    </div>
                </div>
                ...
                <button
                    type="button"
                    class="driver-btn driver-btn--request-update"
                    ...
                    <label>
                        ...
                        <div class="driver-phone-verification__control">
                            <input
                                ...
                                readonly
                                value="{{ $identityProfile->mobile_phone ?: $driver->phone }}"
                                maxlength="16"
            ...
            <div class="driver-form-grid driver-form-grid--2">
                <label>
                    ...
                    <input
                        ...
                        readonly
                        value="{{ $identityProfile->contact_email ?: $driver->email }}"
                        required
                    >
                </label>
            ...
            <div class="driver-form-actions">
                @if ($profileDataLocked)
                    <div class="driver-protected-data-notice driver-protected-data-notice--saved">
                        <div>
                            <span aria-hidden="true">✓</span>

                            <div>
                                <strong>Información guardada y protegida</strong>

                                <small>
                                    Los datos permanecen visibles y no pueden modificarse directamente.
                                    Utiliza “Solicitar actualización” en el bloque superior si necesitas corregirlos.
                                </small>
                            </div>
                        </div>

                        <b>Expediente enviado</b>
                    </div>
                @else
...
<div
    class="driver-tab-panel {{ $activeTab === 'vehiculo' ? 'is-active' : '' }}"
    data-driver-tab-panel="vehiculo"
>
    @include('portals.repartidor.partials.vehicles')
</div>
<div class="driver-tab-panel {{ $activeTab === 'finanzas' ? 'is-active' : '' }}" data-driver-tab-panel="finanzas">
    ...
    <div
        class="driver-update-modal"
        data-driver-update-modal
        hidden
    >
        <button
            type="button"
            class="driver-update-modal__backdrop"
            data-driver-update-request-close
            aria-label="Cerrar solicitud"
        ></button>

        <section
            class="driver-update-modal__dialog"
            role="dialog"
            aria-modal="true"
            aria-labelledby="driver-update-modal-title"
        >
            <header class="driver-update-modal__header">
                <div>
                    <span>Solicitud administrativa</span>

                    <h2 id="driver-update-modal-title">
                        Solicitar actualización
                    </h2>

                    <p>
                        Tus datos actuales permanecerán sin cambios
                        hasta que Admin apruebe la solicitud.
                    </p>
                </div>

                <button
                    type="button"
                    class="driver-update-modal__close"
                    data-driver-update-request-close
                    aria-label="Cerrar"
                >
                    ×
            ...
            <form
                method="POST"
                action="{{ route('repartidor.identity.update-requests.store') }}"
                enctype="multipart/form-data"
                class="driver-update-form"
            >
                @csrf

                <label>
                    <span>Dato que deseas actualizar <b>*</b></span>

                    <select name="field_name" required>
                        <option value="">Selecciona un dato</option>
                        <option value="first_name">Nombre</option>
                        <option value="paternal_last_name">Apellido paterno</option>
                        <option value="maternal_last_name">Apellido materno</option>
                        <option value="email">Correo electrónico</option>
                        <option value="mobile_phone">Teléfono celular</option>
                        <option value="operation_zone">Zona de operación</option>
                        <option value="state">Estado</option>
                        <option value="city">Ciudad o municipio</option>
                        <option value="vehicle_type">Tipo de vehículo</option>
                        <option value="vehicle_make">Marca del vehículo</option>
                        <option value="vehicle_model">Modelo del vehículo</option>
                        <option value="vehicle_plate">Placa</option>
                        <option value="license_number">Número de licencia</option>
                    </select>
                </label>

                <label>
                    <span>Nuevo valor solicitado <b>*</b></span>
                ...
                <label>
                    ...
                    <textarea
                        ...
                        minlength="10"
                        maxlength="1500"
                        placeholder="Explica por qué necesitas modificar este dato."
                        required
                    ></textarea>
                </label>
                ...
                <label>
                    ...
                    <small>
                        Opcional. PDF o imagen de hasta 15 MB.
                    </small>
                </label>

                <div class="driver-update-form__actions">
                    <button
                        type="button"
                        class="driver-btn driver-btn--secondary"
                        data-driver-update-request-close
            ...
            @if ($updateRequests->isNotEmpty())
                <section class="driver-update-history">
                    <header>
                        <h3>Solicitudes recientes</h3>
                        <span>{{ $updateRequests->count() }}</span>
                    </header>

                    @foreach ($updateRequests as $updateRequest)
                        @php
                            $requestFieldLabel = match ($updateRequest->field_name) {
                                'first_name' => 'Nombre',
                                ...
                                'vehicle_plate' => 'Placa',
                                'license_number' => 'Número de licencia',
                                default => $updateRequest->field_name,
                            };

                            $requestStatusLabel = match ($updateRequest->status) {
                                'approved' => 'Aprobada',
                                'rejected' => 'Rechazada',
                        ...
                        <article class="driver-update-history__item">
                            <div>
                                <strong>{{ $requestFieldLabel }}</strong>

                                <span>
                                    {{ $updateRequest->current_value ?: 'Sin valor' }}
                                    →
                                    {{ $updateRequest->requested_value }}
                                </span>
                            ...
                            <b class="is-{{ $updateRequest->status }}">
                                {{ $requestStatusLabel }}
                            </b>
                        </article>
                    @endforeach
