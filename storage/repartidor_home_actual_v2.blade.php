...
<style id="petpay-partner-driver-design">
    :root {
        --partner-cream: #f7f3e8;
        --partner-navy: #061a36;
        --partner-border: #9badc2;
        --partner-blue: #4c98df;
        --partner-muted: #777f8b;
    }

    body:has([data-driver-dashboard]) {
    ...
    .driver-section-heading--spaced {
        margin-top: 58px !important;
    }

    .driver-section-heading h2 {
        margin: 0 !important;
        color: var(--partner-navy) !important;
    ...
    .driver-form input:not([type="checkbox"]),
    .driver-form select,
    ...
    .driver-form textarea:focus {
        border-color: var(--partner-blue) !important;
        outline: 2px solid rgba(76, 152, 223, .14) !important;
    }
    ...
    .driver-btn--primary {
        ...
        border: 0 !important;
        border-radius: 7px !important;
        background: var(--partner-blue) !important;
        color: #fff !important;
        box-shadow: none !important;
    ...
    .driver-doc-item.is-partner-open .driver-document-workspace,
    .driver-doc-item.is-partner-open .driver-doc-note {
        display: grid;
    }

    .driver-document-workspace {
        padding: 14px !important;
        border-top: 1px solid rgba(155, 173, 194, .45) !important;
    }
    ...
    .driver-submit-box {
        ...
        padding: 14px !important;
        border: 1px solid #d5d3ca !important;
        border-radius: 8px !important;
        background: transparent !important;
    }

    .driver-partner-accordion {
    ...
    .driver-partner-accordion__trigger {
        display: flex !important;
        width: 100% !important;
        min-height: 68px !important;
        align-items: center !important;
    ...
    .driver-partner-accordion__trigger strong {
        font-size: 15px !important;
    }

    .driver-partner-accordion__trigger span {
        transition: transform .2s ease;
    }

    .driver-partner-accordion.is-open
    .driver-partner-accordion__trigger span {
        transform: rotate(180deg);
    }

    .driver-partner-accordion__content {
        display: none;
        padding: 16px 0 20px;
        border-top: 1px solid rgba(155, 173, 194, .45);
    }
...
<script id="petpay-partner-driver-behavior">
document.addEventListener('DOMContentLoaded', function () {
    const dashboard = document.querySelector('[data-driver-dashboard]');
    if (!dashboard) return;

    document.querySelectorAll('.driver-doc-item').forEach(function (card, index) {
        const head = card.querySelector('.driver-doc-item__head');
        if (!head) return;

        if (index === 0) card.classList.add('is-partner-open');

        head.addEventListener('click', function (event) {
            if (event.target.closest('button, a, input, label')) return;
            card.classList.toggle('is-partner-open');
        });
    });

    const headings = Array.from(
        dashboard.querySelectorAll('.driver-section-heading')
    );

    const contactsHeading = headings.find(function (heading) {
        return heading.textContent.includes('Contactos de emergencia');
    });

    const referencesHeading = headings.find(function (heading) {
        return heading.textContent.includes('Referencias personales');
    });

    const documentsHeading = headings.find(function (heading) {
        return heading.textContent.includes('Documentación personal');
    });

    const contactsForm = contactsHeading?.nextElementSibling;
    const referencesForm = referencesHeading?.nextElementSibling;
    const documentsList = documentsHeading?.nextElementSibling;
    const submitBox = documentsList?.nextElementSibling;
    ...
    if (
        ...
        contactsHeading.parentNode.insertBefore(
            documentsHeading,
            contactsHeading
        );
        ...
        if (submitBox?.classList.contains('driver-submit-box')) {
            contactsHeading.parentNode.insertBefore(
                submitBox,
                contactsHeading
            );
    ...
    [
        ...
        if (!item.heading || !item.form) return;

        const wrapper = document.createElement('section');
        wrapper.className = 'driver-partner-accordion';

        const title = item.heading.querySelector('h2')?.textContent || '';
        const subtitle = item.heading.querySelector('p')?.textContent || '';

        wrapper.innerHTML =
            '<button type="button" class="driver-partner-accordion__trigger">' +
                '<strong>' + item.icon + ' &nbsp;' + title + '</strong>' +
                '<span>⌄</span>' +
            '</button>' +
            '<div class="driver-partner-accordion__content"></div>';

        item.heading.parentNode.insertBefore(wrapper, item.heading);

        wrapper
            .querySelector('.driver-partner-accordion__content')
            .appendChild(item.form);

        if (subtitle) {
            wrapper
                .querySelector('.driver-partner-accordion__trigger strong')
                .title = subtitle;
        }
        ...
        wrapper
            .querySelector('.driver-partner-accordion__trigger')
